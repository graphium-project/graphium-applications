...
public class StopCsvWriter<T extends AbstractPlace> {
	...
	private void writeHeader(CSVPrinter stopPrinter) throws IOException {
		...
		final Object[] FILE_HEADER = { "fid", "track_id", "placeId", "placeGeom", "stayId", "stayGeom", "startTime",
				...
				"distanceToHighLevelRoad", "frcOfNextHighLevelRoad", "fowOfNextHighLevelRoad", "idOfNextHighLevelRoad",
				"distanceToLowLevelRoad", "frcOfNextLowLevelRoad","fowOfNextLowLevelRoad", "idOfNextLowLevelRoad", "category", };
	...
	private void writeStays(CSVPrinter csvFilePrinter, List<T> places) throws IOException {
		...
		for (T place : places) {
			for (Stay stay : place.getStays()) {
				...
				record.add((stay.getNextHighLevelRoadSegment() != null) ? stay.getNextHighLevelRoadSegment().getFrc() : "");
				record.add((stay.getNextHighLevelRoadSegment() != null) ? stay.getNextHighLevelRoadSegment().getFormOfWay() : "");
				...
				record.add((stay.getNextLowLevelRoadSegment() != null) ? stay.getNextLowLevelRoadSegment().getFrc() : "");
				record.add((stay.getNextLowLevelRoadSegment() != null) ? stay.getNextLowLevelRoadSegment().getFormOfWay() : "");
